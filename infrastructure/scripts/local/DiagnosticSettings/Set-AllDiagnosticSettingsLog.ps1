...
if (!($tagValue)) {
    if (!($resourceType)) {
        Write-Output "Getting a list of all resources."
        $resources = Get-AzResource -ODataQuery $odataFilter
    } 
    elseif ($resourceType) {
        Write-Output "Getting resources matching resource type [$($resourceType)]."
...
foreach ($resource in $resources) {
    ...
    if (!($diagSettings.WorkspaceId)) {
        ...
        $jobs += Start-Job -ScriptBlock $SetAzDiagnosticSettingsJob -ArgumentList $diagnosticSettingName, $workspaceId, $resource
