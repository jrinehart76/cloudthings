...
if ($tagValue) {
    Write-Output "Getting SQL resources matching tag and value [$($tagName)] [$($tagValue)]."
    $tagTable = @{$tagName = $tagValue}
    $resourceGroups = Get-AzResourceGroup -Tag $tagTable -Location $region
    foreach ($resourceGroup in $resourceGroups) {
        $list = Get-AzResource -ResourceGroupName $resourceGroup.ResourceGroupName -ResourceType 'Microsoft.sql/servers' -ODataQuery $odataFilter
        if ($list) {
            $resources += $list
        }
    }
...
if (!($tagValue)) {
    Write-Output "Getting all SQL resources."
