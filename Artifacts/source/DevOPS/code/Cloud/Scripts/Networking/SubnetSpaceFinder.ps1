...
$sublist = $availableips | foreach-Object {($_.split('.') | select -Index 0,1,2)  -join "." } | select -Unique
...
if ($SubnetSize) {
    if (!($subnetoutput | where-Object {$_.maxsubnetsize -le $SubnetSize})) {
    write-host "No subnets large enough for the requested size"
    }
    else {
    $subnetoutput | where-Object {$_.maxsubnetsize -le $SubnetSize}
    }
}
else {
    $subnetoutput
