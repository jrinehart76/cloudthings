...
def main(output_path=None, return_json_output="True"):
    if os.geteuid() != 0:
...
def check_os_version():
    ...
    if re.search("Ubuntu-14.04", os_version, re.IGNORECASE):
        versionname  = "Ubuntu-14.04"
        write_log_output(rule_id, rule_group_id, versionname, empty_failure_reason, "Operating system version is supported")
    elif re.search("Ubuntu-16.04", os_version, re.IGNORECASE):
        versionname  = "Ubuntu-16.04"
        write_log_output(rule_id, rule_group_id, versionname, empty_failure_reason, "Operating system version is supported")
    elif re.search("Ubuntu-18.04", os_version, re.IGNORECASE):
        versionname  = "Ubuntu-18.04"
        write_log_output(rule_id, rule_group_id, versionname, empty_failure_reason, "Operating system version is supported")
    elif re.search("SuSE-11", os_version, re.IGNORECASE):
        versionname  = "SuSE-11"
        write_log_output(rule_id, rule_group_id, versionname, empty_failure_reason, "Operating system version is supported")
    elif re.search("SuSE-12", os_version, re.IGNORECASE):
        versionname  = "SuSE-12"
        write_log_output(rule_id, rule_group_id, versionname, empty_failure_reason, "Operating system version is supported")
    elif re.search("redhat-6", os_version, re.IGNORECASE):
        versionname  = "redhat-6"
        write_log_output(rule_id, rule_group_id, versionname, empty_failure_reason, "Operating system version is supported")
    elif re.search("redhat-7", os_version, re.IGNORECASE):
        versionname  = "redhat-7"
        write_log_output(rule_id, rule_group_id, versionname, empty_failure_reason, "Operating system version is supported")
    elif re.search("centos-6", os_version, re.IGNORECASE):
        versionname  = "centos-6"
        write_log_output(rule_id, rule_group_id, versionname, empty_failure_reason, "Operating system version is supported")
    elif re.search("centos-7", os_version, re.IGNORECASE) :
        versionname  = "centos-7"
        write_log_output(rule_id, rule_group_id, versionname, empty_failure_reason, "Operating system version is supported")
    else:
        versionname = "UnsupportedOS"
        log_msg = "Operating System version (%s) is not supported. Supported versions listed here: %s" % (os_version, supported_os_url)
        write_log_output(rule_id, rule_group_id, versionname, empty_failure_reason, log_msg, supported_os_url)
