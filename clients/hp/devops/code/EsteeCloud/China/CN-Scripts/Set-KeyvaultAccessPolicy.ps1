...
# cloudops group perms - ELC-CloudOps
Set-AzKeyVaultAccessPolicy -VaultName $KeyVaultName -ResourceGroupName $ResourceGroup -ObjectId (Get-AzADGroup -SearchString 'ELC-CN-CloudOps')[0].Id -PermissionsToSecrets get,list,set,delete,recover,backup,restore
Set-AzKeyVaultAccessPolicy -VaultName $KeyVaultName -ResourceGroupName $ResourceGroup -ObjectId (Get-AzADGroup -SearchString 'ELC-CN-CloudOps-ManagedServices')[0].Id -PermissionsToSecrets get,list,set,delete,recover,backup,restore
