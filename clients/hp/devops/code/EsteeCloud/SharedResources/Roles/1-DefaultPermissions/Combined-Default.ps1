...
foreach($p in $PrincipalId){
    foreach($r in $ResourceGroupName){
	    ...
	    $ifExists = Get-RoleDefinition -PrincipalId $p -ResourceGroupName $r -ResourceType "Microsoft.Databricks/workspace" -RoleDefinitionName "Contributor"
        If($ifExists -eq $Null)
        {
			Assign-RoleDefinition -PrincipalId $p -ResourceGroupName $r -ResourceType "Microsoft.Databricks/workspace" -RoleDefinitionName "Contributor"
        }else{Write-Output "Databricks Permissions Already Exists"}
		...
		# Data Factory (ADFS)
	    $ifExists = Get-RoleDefinition -PrincipalId $p -ResourceGroupName $r -ResourceType "Microsoft.DataFactory/factories" -RoleDefinitionName "Data Factory Contributor"
        If($ifExists -eq $Null)
        {
			Assign-RoleDefinition -PrincipalId $p -ResourceGroupName $r -ResourceType "Microsoft.DataFactory/factories" -RoleDefinitionName "Data Factory Contributor"
        }else{Write-Output "ADFS Data Factory Contributor Permissions Already Exists"}

		# RG Data Factory Contributor (ADFS)
	    $ifExists = Get-AzRoleAssignment -ObjectId $p -ResourceGroupName $r -RoleDefinitionName "Data Factory Contributor"
        If($ifExists -eq $Null)
        {
            New-AzRoleAssignment -ObjectId $p -ResourceGroupName $r -RoleDefinitionName "Data Factory Contributor"
        }else{Write-Output "RG Data Factory Contributor Permissions Already Exists"}
		...
		# PowerBI Reader
		$ifExists = Get-RoleDefinition -PrincipalId $p -ResourceGroupName $r -ResourceType "Microsoft.PowerBI/workspaceCollections" -RoleDefinitionName "Reader"
        If($ifExists -eq $Null)
        {
		    Assign-RoleDefinition -PrincipalId $p -ResourceGroupName $r -ResourceType "Microsoft.PowerBI/workspaceCollections" -RoleDefinitionName "Reader"
        }else{Write-Output "PowerBI Permissions Already Exists"}
