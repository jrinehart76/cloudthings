...
Set-AzKeyVaultAccessPolicy -VaultName $KeyVaultName -ResourceGroupName $ResourceGroup -ObjectId (Get-AzADGroup -SearchString 'U-ELC-CloudOps')[0].Id -PermissionsToSecrets get,list,set,delete,recover,backup,restore
Set-AzKeyVaultAccessPolicy -VaultName $KeyVaultName -ResourceGroupName $ResourceGroup -ObjectId (Get-AzADGroup -SearchString 'U-ELC-CloudManagedServices')[0].Id -PermissionsToSecrets get,list,set,delete,recover,backup,restore
